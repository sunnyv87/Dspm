...
async function bootstrap() {
  ...
  const kafkaGroupId =
    configService.get<string>('kafka.groupId') || 'auth-service-group';
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('TechD PrivacyOps Auth Service')
    .setDescription(
      'Multi-tenant authentication, authorization, and user management API for the TechD PrivacyOps platform.',
    )
    .setVersion('1.0')
    .addBearerAuth(
      {
  ...
  logger.log(`Swagger docs available at http://localhost:${port}/api/docs/auth`);
}
